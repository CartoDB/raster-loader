...
def rasterio_windows(
    ...
    """Open a raster file with rasterio."""
    with rasterio.open(file_path) as raster_dataset:
        windows = list(raster_dataset.block_windows())
...
def windows_to_records(
    ...
    for _, window in windows:
        ...
        for band, band_name in bands_info:
            ...
            # add the new columns generated by array_t o_record
...
def overview_records(
    ...
    n_overviews = len(overview_factors)
    print(f"Computing overviews for {n_overviews} levels")
    for overview_index in range(0, len(overview_factors)):
        ...
        print(f"Overview {overview_index}: {x_tiles} x {y_tiles} tiles")
