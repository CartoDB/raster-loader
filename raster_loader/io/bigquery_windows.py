...
class BigQueryConnection(DataWarehouseConnection):
    ...
    def upload_windows(self, file_path: str, windows: Iterable, fqn):
        raster_info = rio_cogeo.cog_info(file_path).dict()
        with rasterio.open(file_path) as raster_dataset:
            records = windows_to_records(
                windows, raster_dataset, raster_info
            )
    ...
    def upload_raster(
        ...
        try:
            ...
            #  total_overviews = len(overviews)
            #  print("Total overviews: ", total_overviews)
            #  job = self.upload_records(overviews, fqn)
            #  job.result()
            ...
            if chunk_size is None:
                ...
                with tqdm(total=total_blocks) as pbar:
                    ...
                    for windows in batched(all_windows, chunk_size):
                        with rasterio.open(file_path) as raster_dataset:
                            records = windows_to_records(
                                windows,
                                raster_dataset,
                        ...
                        is_empty_chunk = all([len(r) == 0 for r in records])
                        if not is_empty_chunk:
                            job = self.upload_records(records, fqn)
                            #  job = self.upload_windows(file_path, windows, fqn)
                            job.num_records = len(windows)
                            #  job.num_records = len(records)

                            job.add_done_callback(
                                partial(lambda job: done_callback(job))
                            )
                            jobs.append(job)
